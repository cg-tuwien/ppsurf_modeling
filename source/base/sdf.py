...
def add_samples_to_volume(vol, pos_ms, val):
    ...
    dist_pos_cell_center = math.cartesian_dist(pos_ms, grid_cell_centers_ms)
...
def implicit_surface_to_mesh(query_dist_ms, query_pts_ms,
    ...
    mesh.write_off(volume_out_file, query_pts_ms, np.array([]), colors_vertex=query_pts_color)
    ...
    if volume.min() < 0.0 and volume.max() > 0.0:
        ...
        if v.size == 0 and f.size == 0:
            ...
            fs.make_dir_for_file(mc_out_file)
...
def implicit_surface_to_mesh_directory(imp_surf_dist_ms_dir, query_pts_ms_dir,
    ...
    for fi, f in enumerate(dist_ps_files):
        ...
        if fs.call_necessary([files_dist_ms_in_abs[fi], files_query_pts_ms_in_abs[fi]],
    ...
    mp.start_process_pool(implicit_surface_to_mesh_file, calls, num_processes)
